...
public class ApprovalService {
	...
	public List<Map<String, Object>> get_list_info(Map<String, Object> get_empl_info,String filter,String type,List<Map<String,Object>> list_params, String list_type) {
		...
		//받은 결재일 경우, doc_idx 파라메터 구하는 로직 추가
		if(list_type.equals("받은 결재")){

			List <String> received_doc_idxs = approvalDAO.get_received_doc_idxs(r_empl_idx);
		...
		for (Map<String, Object> doc_info : doc_infos) {
			...
			distinct_list_filter(filter,type,list_param,list_params);
	...
	public void distinct_list_filter(String filter,String type, Map<String, Object> sent_list_param, List<Map<String, Object>> sent_list_params) {
		switch (type) {
			case "전체":
				sent_list_params.add(sent_list_param);
				break;
			case "탭":
				logger.info("탭");
				if (sent_list_param.get("approval_state").equals(filter)) { //approval_state가 결재 대기일 때만 param 넣기
					sent_list_params.add(sent_list_param);
				}
				break;
			case "검색":
				logger.info("검색");
				if(sent_list_param.get("form_subject").equals(filter)){
					sent_list_params.add(sent_list_param);
				}
				break;
		}
